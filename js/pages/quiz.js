// ============================================================
//  QUIZ PAGE v2 — no nested template literals
// ============================================================
const QuizPage = (() => {
  let questions = [];
  let currentIdx = 0;
  ...
  function render() {
    ...
    const weakBar = topWeak.length
      ? '<div class="weak-topics-bar"><i class="ti ti-flame"></i> Weak areas: ' +
        topWeak.map(function(t){return '<span class="topic-badge warn">'+t+'</span>';}).join(' ') +
        '</div>'
      : '';

    const startCard = questions.length ? '' :
      '<div class="quiz-start-card">' +
        '<div class="quiz-start-icon"><i class="ti ti-brain"></i></div>' +
        '<h2>Ready for today\'s quiz?</h2>' +
        '<p>10 questions &middot; ' + (hasKey ? ['','Easy','Medium','Hard'][diff] : 'Easy') + ' difficulty &middot; Adapts to your weak areas</p>' +
        (hasKey
          ? '<button class="btn primary btn-lg" onclick="QuizPage.startSession()"><i class="ti ti-player-play"></i> Start Quiz</button>'
          : '<div id="quiz-key-msg"></div>') +
      '</div>';
    ...
    if (!hasKey) {
      var msg = document.getElementById('quiz-key-msg');
  ...
  async function startSession() {
    ...
    var startCard = document.querySelector('.quiz-start-card');
    if (startCard) startCard.style.display = 'none';
    UI.loading('quiz-question-area', 'Generating 10 questions with Gemini AI...');
    try {
      var q = await Gemini.generateQuiz(DATA.quizTopics, Store.state.quizDifficulty, Store.state.quizWeakAreas, 10);
  ...
  function renderCurrentQuestion() {
    ...
    var optionsHTML = (q.options || []).map(function(opt, i) {
      ...
      return '<button class="' + cls + '" ' + disabled + ' onclick="QuizPage.selectOption(' + i + ')">' + opt + '</button>';
    }).join('');
    ...
    var explanationHTML = revealed ? buildExplanation(q) : '';
  ...
  function buildExplanation(q) {
    ...
    var wrongHTML = '';
    if (q.wrong_explanations && q.wrong_explanations.length) {
      wrongHTML = '<details class="wrong-details"><summary>Why other options are wrong</summary>' +
        q.wrong_explanations.map(function(w){return '<p>&bull; ' + w + '</p>';}).join('') +
        '</details>';
    }
  ...
  function selectOption(idx) {
    ...
    if (grid && questions.length) {
      ...
      grid.innerHTML = (q.options || []).map(function(opt, i) {
        var cls = 'option-btn' + (i === selectedOption ? ' selected' : '');
        return '<button class="' + cls + '" onclick="QuizPage.selectOption(' + i + ')">' + opt + '</button>';
      }).join('');
    }
  ...
  function renderSessionResult() {
    ...
    var msg = pct >= 80
      ? 'Excellent! Difficulty increases next session.'
      : pct >= 60
        ? 'Good job! Keep practicing.'
        : 'Keep going &mdash; difficulty may ease to help you build foundations.';
